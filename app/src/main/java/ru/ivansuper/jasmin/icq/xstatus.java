...
/**
 * Utility class for handling X-Status (extended status) messages in ICQ.
 * This class provides methods for:
 * <ul>
 *     <li>Mapping X-Status GUIDs to corresponding icons.</li>
 *     <li>Retrieving the index of an X-Status GUID.</li>
 *     <li>Escaping special characters in X-Status prompts for XML compatibility.</li>
 *     <li>Unescaping XML entities in X-Status text.</li>
 *     <li>Extracting content from a specific tag within an XML string.</li>
 * </ul>
 */
public class xstatus {
    ...
    public static String getText(String text) {
        String oldText = null;
        while (equals(oldText, text)) {
            oldText = text;
            text = text.replace("&gt;", ">");
        }
        String oldText2 = null;
        while (equals(oldText2, text)) {
            oldText2 = text;
            text = text.replace("&lt;", "<");
        }
        String oldText3 = null;
        while (equals(oldText3, text)) {
            oldText3 = text;
            text = text.replace("&amp;", "&");
        }
        String oldText4 = null;
        while (equals(oldText4, text)) {
            oldText4 = text;
            text = text.replace("&apos;", "'");
        }
        return text;
